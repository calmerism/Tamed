...
import com.tamed.music.innertube.NewPipeUtils
...
import com.tamed.music.innertube.models.YouTubeClient.Companion.IOS
import com.tamed.music.innertube.models.YouTubeClient.Companion.TVHTML5_SIMPLY_EMBEDDED_PLAYER
import com.tamed.music.innertube.models.YouTubeClient.Companion.WEB_REMIX
import com.tamed.music.innertube.models.response.PlayerResponse
import com.tamed.music.innertube.models.YouTubeClient.Companion.ANDROID_CREATOR
import com.tamed.music.innertube.models.YouTubeClient.Companion.ANDROID_MUSIC
import com.tamed.music.innertube.models.YouTubeClient.Companion.ANDROID_TESTSUITE
import com.tamed.music.innertube.models.YouTubeClient.Companion.ANDROID_UNPLUGGED
import com.tamed.music.innertube.models.YouTubeClient.Companion.ANDROID_VR_1_43_32
import com.tamed.music.innertube.models.YouTubeClient.Companion.ANDROID_VR_1_61_48
import com.tamed.music.innertube.models.YouTubeClient.Companion.ANDROID_VR_NO_AUTH
import com.tamed.music.innertube.models.YouTubeClient.Companion.IPADOS
import com.tamed.music.innertube.models.YouTubeClient.Companion.IOS_MUSIC
import com.tamed.music.innertube.models.YouTubeClient.Companion.MOBILE
import com.tamed.music.innertube.models.YouTubeClient.Companion.TVHTML5
import com.tamed.music.innertube.models.YouTubeClient.Companion.VISIONOS
import com.tamed.music.innertube.models.YouTubeClient.Companion.WEB
import com.tamed.music.innertube.models.YouTubeClient.Companion.WEB_CREATOR
...
object YTPlayerUtils {
     ...
     *
     * [com.tamed.music.innertube.models.YouTubeClient.WEB_REMIX] should be preferred here because currently it is the only client which provides:
        ...
        if (context != null) {
            ...
            if (saavnEnabled) {
                ...
                val saavnResult = runCatching {
                    ...
                    Timber.tag(logTag).d("Saavn: resolved title=\"$title\" artists=$artistNames for videoId=$videoId")
                    ...
                    suspend fun findMatch(searchQuery: String): com.music.jiosaavn.SaavnSong? {
                        ...
                        Timber.tag(logTag).d("Saavn: searching with query: \"$searchQuery\"")
                        val songs = com.music.jiosaavn.SaavnService.searchSongs(searchQuery).getOrNull() ?: return null
                        return songs.firstOrNull { candidate ->
                            ...
                            if (isMatch) {
                                Timber.tag(logTag).d("Saavn: candidate matched: \"${candidate.name}\" on album \"${candidate.album?.name}\" by ${candidate.artists.primary.joinToString { it.name }}")
                            } else {
                                Timber.tag(logTag).d("Saavn: candidate rejected (name/artist mismatch): \"${candidate.name}\" on album \"${candidate.album?.name}\" by ${candidate.artists.primary.joinToString { it.name }}")
                            }
                            isMatch
                        }
                    }
                    ...
                    if (bestSong == null && primaryQuery != fallbackQuery) {
                        Timber.tag(logTag).d("Saavn: no match found with primary query, trying fallback: \"$fallbackQuery\"")
                        bestSong = findMatch(fallbackQuery)
                    }

                    if (bestSong == null) {
                        Timber.tag(logTag).d("Saavn: no matching candidate found")
                        return@runCatching null
                    }

                    Timber.tag(logTag).i("Saavn: matched \"${bestSong.name}\" (id=${bestSong.id}, album=\"${bestSong.album?.name}\")")

                    val saavnQualityStr = context.dataStore.get(SaavnAudioQualityKey, SaavnAudioQuality.QUALITY_320.name)
                    val saavnQuality = runCatching { SaavnAudioQuality.valueOf(saavnQualityStr) }
                    ...
                    var streamUrl = com.music.jiosaavn.SaavnService.selectBestUrl(bestSong.downloadUrl, saavnQuality.toApiValue())
                    if (streamUrl.isNullOrBlank()) {
                        Timber.tag(logTag).d("Saavn: downloadUrl list empty in search results, fetching via getBestStreamUrl for songId=${bestSong.id}")
                        streamUrl = com.music.jiosaavn.SaavnService.getBestStreamUrl(bestSong.id, saavnQuality.toApiValue())
                    } else {
                        Timber.tag(logTag).d("Saavn: resolved stream URL directly from search results: $streamUrl")
                    }

                    if (streamUrl.isNullOrBlank()) {
                        Timber.tag(logTag).d("Saavn: no stream URL for songId=${bestSong.id}")
                        return@runCatching null
                    }

                    // Issue a HEAD request to get the content length of the stream so that FormatEntity and Downloader have exact content length
                    val contentLength = com.music.jiosaavn.SaavnService.getContentLength(streamUrl) ?: 0L

                    Timber.tag(logTag).i("Saavn: streaming from JioSaavn (quality=${saavnQuality.toApiValue()}) for videoId=$videoId")
                    PlaybackData(
                        audioConfig      = meta?.playerConfig?.audioConfig,
                        videoDetails     = meta?.videoDetails,
                        playbackTracking = meta?.playbackTracking,
                        format           = PlayerResponse.StreamingData.Format(
                            itag             = when (saavnQuality) {
                            ...
                            quality          = saavnQuality.toApiValue(),
                            fps              = null,
                            qualityLabel     = null,
                            averageBitrate   = null,
                            audioQuality     = saavnQuality.toApiValue(),
                ...
                Timber.tag(logTag).w("Saavn streaming failed or matching song not found; falling back to YouTube Music")
            }
        }
        val attempts =
            when (audioQuality) {
