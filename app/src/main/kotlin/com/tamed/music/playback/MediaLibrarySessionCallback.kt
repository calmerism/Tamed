...
import com.tamed.music.constants.MediaSessionConstants
                                ...
                                when (val playlistId =
                                    ...
                                    PlaylistEntity.DOWNLOADED_PLAYLIST_ID -> {
                                        ...
                                        database
                                            ...
                                            .map { songs ->
                                                songs.filter {
                                                    it.song.isLocal || downloads[it.id]?.state == Download.STATE_COMPLETED
                                                }
                                            }.map { songs ->
                                                songs
                                                    .sortedBy { song ->
                                                        if (song.song.isLocal) {
                                                            song.song.dateDownload?.toInstant(java.time.ZoneOffset.UTC)?.toEpochMilli() ?: 0L
                                                        } else {
