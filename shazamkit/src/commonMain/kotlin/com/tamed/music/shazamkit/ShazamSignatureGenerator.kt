...
class ShazamSignatureGenerator(
    ...
    fun nextSignatureOrNull(): ShazamSignature? {
        ...
        val message = DecodedMessage(
            ...
            frequencyBandToSoundPeaks = bandToPeaks.entries
    ...
    private fun doPeakSpreading() {
        ...
        originSpread.copyInto(spreadOutputs[spreadPos], endIndex = 1025)
            ...
            if (energy >= (1.0 / 64.0) && energy >= spreadMinus49[bin - 1]) {
                ...
                if (energy > maxNeighbor) {
                    ...
                    if (energy > maxTimeNeighbor) {
                        ...
                        val peakMagnitudeBefore = ln(max(1.0 / 64.0, fftMinus46[bin - 1])) * 1477.3 + 6144.0
                        val peakMagnitudeAfter = ln(max(1.0 / 64.0, fftMinus46[bin + 1])) * 1477.3 + 6144.0
    ...
    private data class DecodedMessage(
        ...
        private fun encodeToBinary(): ByteArray {
            ...
            for ((band, peaks) in frequencyBandToSoundPeaks.entries.sortedBy { it.key.value }) {
                ...
                contents.writeBytes(peaksValue)
                val padding = (-peaksValue.size).floorMod(4)
                repeat(padding) { contents.writeByte(0) }
            }

            val contentsValue = contents.toByteArray()
            val sizeMinusHeader = contentsValue.size + 8

            val full = BinaryWriter()
            full.writeLittleInt(0xCAFE2580.toInt())
            full.writeLittleInt(0)
            full.writeLittleInt(sizeMinusHeader)
            full.writeLittleInt(0x94119C00.toInt())
            full.writeLittleInt(0)
            full.writeLittleInt(0)
            full.writeLittleInt(0)
            val sampleRateId = 3
            full.writeLittleInt(sampleRateId shl 27)
            full.writeLittleInt(0)
            full.writeLittleInt(0)
            full.writeLittleInt((numberSamples + sampleRateHz * 0.24).toInt())
            full.writeLittleInt((15 shl 19) + 0x40000)
            full.writeLittleInt(0x40000000)
            full.writeLittleInt(sizeMinusHeader)
            full.writeBytes(contentsValue)

            val withHeader = full.toByteArray()
            val crc = crc32(withHeader, start = 8, endExclusive = withHeader.size)
            writeLittleIntAt(withHeader, 4, crc)
            return withHeader
        }
    }
}
...
private class BinaryWriter(initialCapacity: Int = 256) {
    ...
    fun writeLittleShort(value: Int) {
        writeByte(value and 0xFF)
        writeByte((value ushr 8) and 0xFF)
    }
...
private fun crc32(
    bytes: ByteArray,
    start: Int = 0,
    endExclusive: Int = bytes.size,
): Int {
    var crc = 0xFFFFFFFF.toInt()
    for (index in start until endExclusive) {
        var current = (crc xor (bytes[index].toInt() and 0xFF)) and 0xFF
        repeat(8) {
            current = if ((current and 1) != 0) {
                0xEDB88320.toInt() xor (current ushr 1)
            } else {
                current ushr 1
            }
        }
        crc = (crc ushr 8) xor current
    }
    return crc.inv()
}

private fun writeLittleIntAt(
    bytes: ByteArray,
    offset: Int,
    value: Int,
) {
...
private class Fft(private val n: Int) {
    ...
    private val bitReversal = IntArray(n).also { out ->
        val bits = n.countTrailingZeroBits()
        for (i in 0 until n) {
