...
import com.tamed.music.constants.AlbumSortType
import com.tamed.music.constants.ArtistSongSortType
import com.tamed.music.constants.ArtistSortType
import com.tamed.music.constants.PlaylistSortType
import com.tamed.music.constants.SongSortType
...
import com.tamed.music.db.entities.PlayCountEntity
...
import com.tamed.music.db.entities.EventWithSong
...
import com.tamed.music.db.entities.PlaylistPlayCount
...
import com.tamed.music.db.entities.SetVideoIdEntity
...
import com.tamed.music.db.entities.SongWithStats
import com.tamed.music.db.entities.TagEntity
import com.tamed.music.db.entities.PlaylistTagMap
import com.tamed.music.db.entities.PlaylistWithTags
import com.tamed.music.extensions.reversed
import com.tamed.music.extensions.toSQLiteQuery
...
interface DatabaseDao {
    ...
    @Transaction
    @Query("SELECT * FROM song WHERE isDownloaded = 1 ORDER BY dateDownload, song.rowid")
    ...
    @Query(
        ...
        FROM song
        LEFT JOIN set_video_id ON set_video_id.videoId = song.id
        WHERE isDownloaded = 1 AND set_video_id.setVideoId IS NULL
        ORDER BY dateDownload, song.rowid
        """,
    )
    fun downloadedSongsByCreateDateAscNoVideo(): Flow<List<Song>>
    ...
    fun downloadedSongsByNameAscNoVideo(): Flow<List<Song>>
    ...
    fun downloadedSongsByPlayTimeAscNoVideo(): Flow<List<Song>>
    ...
    fun downloadedSongsByRowIdAscNoVideo(): Flow<List<Song>>
    ...
    fun downloadedSongs(
        ...
        SongSortType.CREATE_DATE ->
            if (filterVideo) {
                downloadedSongsByCreateDateAscNoVideo()
            } else {
        ...
        SongSortType.NAME ->
            (if (filterVideo) {
                downloadedSongsByNameAscNoVideo()
            } else {
        ...
        SongSortType.ARTIST ->
            (if (filterVideo) {
                downloadedSongsByRowIdAscNoVideo()
            } else {
                downloadedSongsByRowIdAsc()
            }).map { songs ->
                val collator = Collator.getInstance(Locale.getDefault())
                collator.strength = Collator.PRIMARY
                songs
                    ...
                    .flatMap { (_, songsByAlbum) ->
                        songsByAlbum.sortedBy { album ->
                            val collatorAlbum = Collator.getInstance(Locale.getDefault())
                            collatorAlbum.strength = Collator.PRIMARY
                            album.song.title
                        }
                    }
            }
        ...
        SongSortType.PLAY_TIME ->
            if (filterVideo) {
                downloadedSongsByPlayTimeAscNoVideo()
            } else {
