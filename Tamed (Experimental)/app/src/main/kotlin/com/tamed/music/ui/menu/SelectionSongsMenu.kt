...
fun SelectionSongMenu(
    ...
    var showLosslessSheet by remember { mutableStateOf(false) }
...
fun SelectionMediaMetadataMenu(
    ...
    if (showLosslessSheet) {
        LosslessDownloadSheet(
            title = stringResource(R.string.selection),
            subtitle = pluralStringResource(R.plurals.n_song, songSelection.size, songSelection.size),
            trackCount = songSelection.size,
            onDismiss = { showLosslessSheet = false },
            enableYouTubeDownload = true,
            onStartYouTube = {
                showLosslessSheet = false
                songSelection.forEach { s ->
                    val request = DownloadRequest.Builder(s.id, s.id.toUri())
                        .setCustomCacheKey(s.id)
                        .setData(s.title.toByteArray())
                        .build()
            ...
            onStart = { provider, quality ->
                ...
                App.launchOnAppScope {
                    val imported =
                        LocalFlacLibraryImporter.importMatchingMediaMetadata(
                    ...
                    if (imported.imported < imported.total) {
                        val itemsToDownload = songSelection.filter { it.id !in imported.matchedIds }
                        val results = itemsToDownload.map { item ->
                            SpotiFlacDownloader.downloadMediaMetadata(
                                context = context.applicationContext,
                                database = database,
                                mediaMetadata = item,
                                provider = provider,
                        ...
                        if (backendMissing) {
                            Toast.makeText(
                                context, 
                                context.getString(R.string.flac_backend_missing_multiple),
                                Toast.LENGTH_LONG
                            ).show()
                        }
                    }
