...
func TestQobuzAvailableProviders(t *testing.T) {
	...
	want := map[string]string{
		"jumo":  qobuzAPIKindJumo,
		"kenny": qobuzAPIKindKenny,
		"squid": qobuzAPIKindSquid,
	}
...
func TestQobuzProvidersForBackend(t *testing.T) {
	downloader := NewQobuzDownloader()
	cases := map[string][]string{
		"":      {"jumo", "kenny", "squid"},
		"jumo":  {"jumo", "kenny", "squid"},
		"kenny": {"kenny", "jumo", "squid"},
		"squid": {"squid", "kenny", "jumo"},
	}

	for backend, want := range cases {
		providers := downloader.providersForBackend(backend)
		if len(providers) != len(want) {
			t.Fatalf("backend %q returned %d providers, want %d", backend, len(providers), len(want))
		}
		for i, provider := range providers {
			if provider.Name != want[i] {
				t.Fatalf("backend %q provider %d = %q, want %q", backend, i, provider.Name, want[i])
			}
		}
	}
}
...
func TestResolveQobuzTrackForRequestRejectsSongLinkMismatch(t *testing.T) {
	...
	qobuzSearchTrackByISRCWithDurationFunc = func(_ *QobuzDownloader, isrc string, expectedDurationSec int, _, _ string) (*QobuzTrack, error) {
		if isrc != "TESTISRC1" {
	...
	qobuzSearchTrackByMetadataWithDurationFunc = func(_ *QobuzDownloader, _, _ string, _ int, _, _ string) (*QobuzTrack, error) {
...
func TestResolveQobuzTrackForRequestRejectsOdesliMismatch(t *testing.T) {
	...
	qobuzSearchTrackByISRCWithDurationFunc = func(_ *QobuzDownloader, _ string, _ int, _, _ string) (*QobuzTrack, error) {
...
func TestNormalizeQobuzBackendName(t *testing.T) {
	tests := map[string]string{
		"":        "JUMO",
		"jumo":    "JUMO",
		"KENNY":   "KENNY",
	...
	for input, want := range tests {
		if got := normalizeQobuzBackendName(input); got != want {
			t.Fatalf("normalizeQobuzBackendName(%q) = %q, want %q", input, got, want)
		}
	}
}

func TestNormalizeQobuzCountryCode(t *testing.T) {
	tests := map[string]string{
		"":     "US",
		"us":   "US",
		" fr ": "FR",
	...
	for input, want := range tests {
		if got := normalizeQobuzCountryCode(input); got != want {
			t.Fatalf("normalizeQobuzCountryCode(%q) = %q, want %q", input, got, want)
		}
	}
}

func TestNormalizeQobuzResolverRegion(t *testing.T) {
	tests := []struct {
		country  string
		provider string
		want     string
	}{
		{country: "us", provider: "jumo", want: "FR"},
		{country: "jp", provider: "jumo", want: "JP"},
		{country: "fr", provider: "kenny", want: "FR"},
		{country: "bad", provider: "squid", want: "US"},
	}

	for _, test := range tests {
		if got := normalizeQobuzResolverRegion(test.country, test.provider); got != test.want {
			t.Fatalf("normalizeQobuzResolverRegion(%q, %q) = %q, want %q", test.country, test.provider, got, test.want)
		}
	}
}
