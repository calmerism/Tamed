...
var (
	...
	qobuzSearchTrackByISRCWithDurationFunc = func(q *QobuzDownloader, isrc string, expectedDurationSec int, backend, country string) (*QobuzTrack, error) {
		return q.SearchTrackByISRCWithDurationAndOptions(isrc, expectedDurationSec, backend, country)
	}
	qobuzSearchTrackByMetadataWithDurationFunc = func(q *QobuzDownloader, trackName, artistName string, expectedDurationSec int, backend, country string) (*QobuzTrack, error) {
		return q.SearchTrackByMetadataWithDurationAndOptions(trackName, artistName, expectedDurationSec, backend, country)
	}
...
const (
	...
	qobuzJumoDownloadAPIURL  = "https://jumo-dl.pages.dev/fetch"
	qobuzKennyDownloadAPIURL = "https://qobuz.kennyy.com.br/api/download-music"
	qobuzKennySearchAPIURL   = "https://qobuz.kennyy.com.br/api/get-music"
	qobuzSquidDownloadAPIURL = "https://qobuz.squid.wtf/api/download-music"
	qobuzSquidSearchAPIURL   = "https://qobuz.squid.wtf/api/get-music"
...
const (
	qobuzAPIKindJumo  = "jumo"
	qobuzAPIKindKenny = "kenny"
	qobuzAPIKindSquid = "squid"
)

const qobuzResolverMobileUserAgent = "Mozilla/5.0 (Linux; Android 14; Pixel 8 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/135.0.0.0 Mobile Safari/537.36"
...
func isQobuzMaintenanceError(err error) bool {
	if err == nil {
		return false
	}
...
func (q *QobuzDownloader) GetAvailableAPIs() []string {
	return []string{
		qobuzJumoDownloadAPIURL,
		qobuzKennyDownloadAPIURL,
		qobuzSquidDownloadAPIURL,
	}
}
...
func (q *QobuzDownloader) GetAvailableProviders() []qobuzAPIProvider {
	return []qobuzAPIProvider{
		{Name: "jumo", URL: qobuzJumoDownloadAPIURL, Kind: qobuzAPIKindJumo},
		{Name: "kenny", URL: qobuzKennyDownloadAPIURL, Kind: qobuzAPIKindKenny},
		{Name: "squid", URL: qobuzSquidDownloadAPIURL, Kind: qobuzAPIKindSquid},
	}
}

func normalizeQobuzBackendName(backend string) string {
	switch strings.ToUpper(strings.TrimSpace(backend)) {
	case "KENNY":
		return "KENNY"
	case "SQUID":
		return "SQUID"
...
func normalizeQobuzCountryCode(country string) string {
	normalized := strings.ToUpper(strings.TrimSpace(country))
	if matched, _ := regexp.MatchString(`^[A-Z]{2}$`, normalized); matched {
		return normalized
	}
	return "US"
}

func normalizeQobuzResolverRegion(country, providerName string) string {
	normalized := strings.ToUpper(strings.TrimSpace(country))
...
func findQobuzProviderByName(providers []qobuzAPIProvider, name string) (qobuzAPIProvider, bool) {
	for _, provider := range providers {
		if strings.EqualFold(provider.Name, name) {
			return provider, true
		}
	}
	return qobuzAPIProvider{}, false
}

func (q *QobuzDownloader) providersForBackend(backend string) []qobuzAPIProvider {
	providers := q.GetAvailableProviders()
	switch normalizeQobuzBackendName(backend) {
	case "SQUID":
		return orderedQobuzProviders(providers, "squid", "kenny", "jumo")
	case "KENNY":
		return orderedQobuzProviders(providers, "kenny", "jumo", "squid")
	}
	return orderedQobuzProviders(providers, "jumo", "kenny", "squid")
}

func orderedQobuzProviders(providers []qobuzAPIProvider, names ...string) []qobuzAPIProvider {
	ordered := make([]qobuzAPIProvider, 0, len(names))
	seen := make(map[string]struct{}, len(names))
	for _, name := range names {
		provider, ok := findQobuzProviderByName(providers, name)
		if !ok {
			continue
		}
...
type qobuzSearchProvider struct {
	Name              string
	URL               string
	UsesCountryHeader bool
}

func qobuzSearchProvidersForBackend(backend string) []qobuzSearchProvider {
	switch normalizeQobuzBackendName(backend) {
	case "SQUID":
		return []qobuzSearchProvider{
			{Name: "squid", URL: qobuzSquidSearchAPIURL, UsesCountryHeader: true},
			{Name: "kenny", URL: qobuzKennySearchAPIURL},
		}
	default:
		return []qobuzSearchProvider{
			{Name: "kenny", URL: qobuzKennySearchAPIURL},
...
type qobuzResolverSearchResponse struct {
...
func buildQobuzResolverRequest(baseURL string, query string, country string, provider qobuzSearchProvider) (*http.Request, error) {
	...
	req.Header.Set("Origin", strings.TrimSuffix(baseURL, "/api/get-music"))
	req.Header.Set("Referer", strings.TrimSuffix(baseURL, "/api/get-music")+"/")
	req.Header.Set("User-Agent", qobuzResolverMobileUserAgent)
	if provider.UsesCountryHeader {
		req.Header.Set("Token-Country", normalizeQobuzResolverRegion(country, provider.Name))
	}
	return req, nil
}

func sortQobuzSearchCandidates(query string, tracks []QobuzTrack, limit int) []QobuzTrack {
	candidates := make([]qobuzTrackSearchCandidate, 0, len(tracks))
	for i := range tracks {
		score := qobuzScoreTrackSearchCandidate(query, &tracks[i])
		if score <= 0 {
			continue
		}
	...
	sorted := make([]QobuzTrack, 0, len(candidates))
	for _, candidate := range candidates {
		sorted = append(sorted, candidate.track)
	}
	return sorted
}

func (q *QobuzDownloader) searchQobuzTracksViaResolverSearch(query string, limit int, backend, country string) ([]QobuzTrack, error) {
	providers := qobuzSearchProvidersForBackend(backend)
	seenTrackIDs := make(map[int64]struct{}, limit)
	combined := make([]QobuzTrack, 0, limit)
	var errors []string

	for _, provider := range providers {
		req, err := buildQobuzResolverRequest(provider.URL, query, country, provider)
		if err != nil {
		...
		resp.Body.Close()
		if readErr != nil {
			errors = append(errors, fmt.Sprintf("%s: %v", provider.Name, readErr))
			continue
		}
		if resp.StatusCode != http.StatusOK {
			errors = append(errors, fmt.Sprintf("%s: HTTP %d (%s)", provider.Name, resp.StatusCode, strings.TrimSpace(string(body))))
			continue
		}

		var payload qobuzResolverSearchResponse
		if err := json.Unmarshal(body, &payload); err != nil {
			errors = append(errors, fmt.Sprintf("%s: invalid JSON: %v", provider.Name, err))
			continue
		}

		if !payload.Success {
			message := qobuzFirstNonEmpty(payload.Error, payload.Message, "success=false")
			errors = append(errors, fmt.Sprintf("%s: %s", provider.Name, message))
			continue
		}

		added := 0
		for i := range payload.Data.Tracks.Items {
			track := payload.Data.Tracks.Items[i]
			if track.ID > 0 {
		...
		if added > 0 {
			GoLog("[Qobuz] MetroFuse-style %s search returned %d tracks for '%s'\n", provider.Name, added, query)
		}
	}

	if len(combined) == 0 {
		if len(errors) == 0 {
			return nil, fmt.Errorf("resolver search returned no tracks")
		}
		return nil, fmt.Errorf("resolver search failed: %s", strings.Join(errors, "; "))
	}

	return sortQobuzSearchCandidates(query, combined, limit), nil
}

func (q *QobuzDownloader) SearchTrackByISRC(isrc string) (*QobuzTrack, error) {
	candidates, err := q.searchQobuzTracksWithOptions(isrc, 50, "", "")
	if err != nil {
		return nil, err
	}
...
func (q *QobuzDownloader) SearchTrackByISRCWithDuration(isrc string, expectedDurationSec int) (*QobuzTrack, error) {
	return q.SearchTrackByISRCWithDurationAndOptions(isrc, expectedDurationSec, "", "")
}

func (q *QobuzDownloader) SearchTrackByISRCWithDurationAndOptions(isrc string, expectedDurationSec int, backend, country string) (*QobuzTrack, error) {
	GoLog("[Qobuz] Searching by ISRC: %s\n", isrc)

	candidates, err := q.searchQobuzTracksWithOptions(isrc, 50, backend, country)
	if err != nil {
		return nil, err
	}
...
func (q *QobuzDownloader) SearchTrackByMetadataWithDuration(trackName, artistName string, expectedDurationSec int) (*QobuzTrack, error) {
	return q.SearchTrackByMetadataWithDurationAndOptions(trackName, artistName, expectedDurationSec, "", "")
}
...
func (q *QobuzDownloader) SearchTrackByMetadataWithDurationAndOptions(trackName, artistName string, expectedDurationSec int, backend, country string) (*QobuzTrack, error) {
	queries := []string{}
	...
	for _, query := range queries {
		...
		result, err := q.searchQobuzTracksWithOptions(cleanQuery, 50, backend, country)
		if err != nil {
...
func (q *QobuzDownloader) searchQobuzTracksWithFallback(query string, limit int) ([]QobuzTrack, error) {
	return q.searchQobuzTracksWithOptions(query, limit, "", "")
}

func (q *QobuzDownloader) searchQobuzTracksWithOptions(query string, limit int, backend, country string) ([]QobuzTrack, error) {
	resolverTracks, resolverErr := q.searchQobuzTracksViaResolverSearch(query, limit, backend, country)
	if resolverErr == nil && len(resolverTracks) > 0 {
		return resolverTracks, nil
	}
	if resolverErr != nil {
		GoLog("[Qobuz] MetroFuse-style resolver search failed for '%s': %v. Trying legacy fallbacks.\n", query, resolverErr)
	}
	...
	if apiErr == nil {
		...
		if isQobuzMaintenanceError(apiErr) {
			return nil, apiErr
		}
	}
	...
	if albumErr != nil {
		...
		if isQobuzMaintenanceError(albumErr) {
			return nil, albumErr
		}
	}
...
func fetchQobuzURLWithRetry(provider qobuzAPIProvider, trackID int64, quality string, timeout time.Duration, country string) (qobuzDownloadInfo, error) {
	return fetchQobuzURLSingleAttempt(provider, trackID, quality, timeout, country)
}
...
func fetchQobuzURLSingleAttempt(provider qobuzAPIProvider, trackID int64, quality string, timeout time.Duration, country string) (qobuzDownloadInfo, error) {
	...
	for attempt := 0; attempt <= qobuzMaxRetries; attempt++ {
		...
		case qobuzAPIKindJumo:
			requestURL, parseErr := url.Parse(provider.URL)
			if parseErr != nil {
				lastErr = parseErr
				continue
			}
			values := requestURL.Query()
			values.Set("track_id", strconv.FormatInt(trackID, 10))
			values.Set("format_id", normalizeQobuzQualityCode(quality))
			values.Set("region", normalizeQobuzResolverRegion(country, provider.Name))
			requestURL.RawQuery = values.Encode()
			req, err = http.NewRequest("GET", requestURL.String(), nil)
			if err == nil {
				req.Header.Set("Accept", "application/json, text/plain, */*")
				req.Header.Set("Accept-Language", "en-US,en;q=0.9")
				req.Header.Set("Origin", "https://jumo-dl.pages.dev")
				req.Header.Set("Referer", "https://jumo-dl.pages.dev/")
				req.Header.Set("User-Agent", qobuzResolverMobileUserAgent)
			}
		case qobuzAPIKindKenny, qobuzAPIKindSquid:
			requestURL, parseErr := url.Parse(provider.URL)
			if parseErr != nil {
				lastErr = parseErr
				continue
			...
			values := requestURL.Query()
			values.Set("track_id", strconv.FormatInt(trackID, 10))
			values.Set("quality", normalizeQobuzQualityCode(quality))
			requestURL.RawQuery = values.Encode()
			req, err = http.NewRequest("GET", requestURL.String(), nil)
			if err == nil {
				origin := "https://qobuz.kennyy.com.br"
				referer := "https://qobuz.kennyy.com.br/"
				if provider.Kind == qobuzAPIKindSquid {
		...
		default:
			return qobuzDownloadInfo{}, fmt.Errorf("unsupported qobuz provider: %s", provider.Name)
		}
		if err != nil {
			lastErr = err
			continue
...
func getQobuzDownloadURLParallel(providers []qobuzAPIProvider, trackID int64, quality string, country string) (qobuzAPIProvider, qobuzDownloadInfo, error) {
	if len(providers) == 0 {
	...
	for _, provider := range providers {
		go func(provider qobuzAPIProvider) {
			reqStart := time.Now()
			info, err := fetchQobuzURLWithRetry(provider, trackID, quality, timeout, country)
...
func (q *QobuzDownloader) GetDownloadURL(trackID int64, quality string) (qobuzDownloadInfo, error) {
	return q.GetDownloadURLWithOptions(trackID, quality, "", "")
}

func (q *QobuzDownloader) GetDownloadURLWithOptions(trackID int64, quality, backend, country string) (qobuzDownloadInfo, error) {
	providers := q.providersForBackend(backend)
	if len(providers) == 0 {
		return qobuzDownloadInfo{}, fmt.Errorf("no Qobuz API available")
	}

	qualityCode := normalizeQobuzQualityCode(quality)
	normalizedBackend := normalizeQobuzBackendName(backend)
	normalizedCountry := normalizeQobuzCountryCode(country)

	downloadFunc := func(qual, countryCode string) (qobuzDownloadInfo, error) {
		provider, info, err := getQobuzDownloadURLParallel(providers, trackID, qual, countryCode)
		if err != nil {
			return qobuzDownloadInfo{}, err
		}
		GoLog("[Qobuz] Download URL resolved via %s (backend=%s, country=%s)\n", provider.Name, normalizedBackend, countryCode)
		return info, nil
	}

	tryDownload := func(qual string) (qobuzDownloadInfo, error) {
		downloadInfo, err := downloadFunc(qual, normalizedCountry)
		if err == nil {
			return downloadInfo, nil
		}
		if normalizedBackend == "JUMO" && normalizedCountry != "FR" {
			downloadInfo, frErr := downloadFunc(qual, "FR")
			if frErr == nil {
				return downloadInfo, nil
			}
			err = frErr
		...
		if normalizedCountry != "" {
			downloadInfo, noCountryErr := downloadFunc(qual, "")
			if noCountryErr == nil {
				return downloadInfo, nil
			}
			err = noCountryErr
	...
	downloadInfo, err := tryDownload(qualityCode)
	if err == nil {
		return downloadInfo, nil
	}
	...
	if currentQuality == "27" {
		...
		downloadInfo, err = tryDownload("7")
		if err == nil {
			return downloadInfo, nil
		}
		currentQuality = "7"
	...
	if currentQuality == "7" {
		...
		downloadInfo, err = tryDownload("6")
		if err == nil {
			return downloadInfo, nil
		}
	}
...
func (q *QobuzDownloader) DownloadFile(downloadURL, outputPath string, outputFD int, itemID string) error {
	...
	resp, err := DoRequestWithUserAgent(GetDownloadClient(), req)
	if err != nil {
		if isDownloadCancelled(itemID) {
			return ErrDownloadCancelled
		}
	...
	resp.Body = NewIdleTimeoutReader(resp.Body, 30*time.Second)
...
func resolveQobuzTrackForRequest(req DownloadRequest, downloader *QobuzDownloader, logPrefix string) (*QobuzTrack, error) {
		...
		if slErr == nil && availability != nil && availability.QobuzID != "" {
			...
			if _, parseErr := fmt.Sscanf(availability.QobuzID, "%d", &trackID); parseErr == nil && trackID > 0 {
				...
				if err != nil {
					...
					if qobuzTrackMatchesRequest(req, track, logPrefix, "SongLink Qobuz ID", false) {
	...
	if track == nil && req.ISRC != "" {
		...
		track, err = qobuzSearchTrackByISRCWithDurationFunc(downloader, req.ISRC, expectedDurationSec, req.QobuzBackend, req.QobuzCountry)
		if track != nil && !qobuzTrackMatchesRequest(req, track, logPrefix, "ISRC search", false) {
			track = nil
		}
	}

	// Strategy 5: Metadata search with duration verification
	if track == nil && req.TrackName != "" && req.ArtistName != "" {
		GoLog("[%s] Trying metadata search: '%s' by '%s'\n", logPrefix, req.TrackName, req.ArtistName)
		track, err = qobuzSearchTrackByMetadataWithDurationFunc(downloader, req.TrackName, req.ArtistName, expectedDurationSec, req.QobuzBackend, req.QobuzCountry)
