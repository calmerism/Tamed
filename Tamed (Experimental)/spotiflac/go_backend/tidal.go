	...
	for attempt := 0; attempt <= tidalMaxRetries; attempt++ {
		...
		if err := json.Unmarshal(body, &v2Response); err == nil && v2Response.Data.Manifest != "" {
			...
			return TidalDownloadInfo{
				...
				ManifestMimeType: strings.TrimSpace(v2Response.Data.ManifestMimeType),
			}, nil
		}
...
func inferTidalDirectMimeType(downloadURL, requestedQuality string) (string, string) {
	lowerURL := strings.ToLower(strings.TrimSpace(downloadURL))
	if strings.HasSuffix(lowerURL, ".m4a") || strings.HasSuffix(lowerURL, ".mp4") || strings.EqualFold(strings.TrimSpace(requestedQuality), "HIGH") {
		return "audio/mp4", "mp4a.40.2"
	}
	return "audio/flac", "flac"
}

func resolveTidalPlaybackStream(downloadInfo TidalDownloadInfo, requestedQuality string) (string, string, string, error) {
	if !strings.HasPrefix(downloadInfo.URL, "MANIFEST:") {
		mimeType, codecs := inferTidalDirectMimeType(downloadInfo.URL, requestedQuality)
		return downloadInfo.URL, mimeType, codecs, nil
	}

	manifestB64 := strings.TrimPrefix(downloadInfo.URL, "MANIFEST:")
	manifestBytes, err := base64.StdEncoding.DecodeString(manifestB64)
	if err != nil {
		return "", "", "", fmt.Errorf("failed to decode tidal manifest: %w", err)
	}
	...
	if strings.HasPrefix(manifestStr, "{") {
		...
		if err := json.Unmarshal(manifestBytes, &manifest); err != nil {
			return "", "", "", fmt.Errorf("failed to parse tidal BTS manifest: %w", err)
		}
		if len(manifest.URLs) == 0 || strings.TrimSpace(manifest.URLs[0]) == "" {
			return "", "", "", fmt.Errorf("tidal BTS manifest did not contain a direct URL")
		}
		mimeType := strings.TrimSpace(manifest.MimeType)
		codecs := strings.TrimSpace(manifest.Codecs)
		if mimeType == "" || codecs == "" {
			fallbackMime, fallbackCodecs := inferTidalDirectMimeType(manifest.URLs[0], requestedQuality)
			if mimeType == "" {
				mimeType = fallbackMime
			}
			if codecs == "" {
				codecs = fallbackCodecs
			}
		}
		return strings.TrimSpace(manifest.URLs[0]), mimeType, codecs, nil
	}

	if strings.TrimSpace(downloadInfo.ManifestMimeType) != "" {
		return "", "", "", fmt.Errorf("tidal manifest playback is not directly supported for %s", downloadInfo.ManifestMimeType)
	}
	return "", "", "", fmt.Errorf("tidal DASH manifests are not directly playable without segment assembly")
}
...
func resolveTidalTrackForRequest(req DownloadRequest, downloader *TidalDownloader, logPrefix string) (*TidalTrack, error) {
	...
	if !gotTidalID && req.TrackName != "" && req.ArtistName != "" {
		GoLog("[%s] Trying Tidal public metadata search without ISRC\n", logPrefix)
		searchTrack, searchErr := downloader.SearchTrackByMetadataWithISRC(
			req.TrackName,
			req.ArtistName,
			req.AlbumName,
		...
		if searchErr == nil && searchTrack != nil && searchTrack.ID > 0 {
			trackID = searchTrack.ID
			gotTidalID = true
			GoLog("[%s] Got Tidal ID %d from public metadata search without ISRC\n", logPrefix, trackID)
		} else if searchErr != nil {
			GoLog("[%s] Tidal public metadata search without ISRC failed: %v\n", logPrefix, searchErr)
		}
	}
