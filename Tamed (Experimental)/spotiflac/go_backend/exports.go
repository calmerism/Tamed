...
type DownloadRequest struct {
	...
	QobuzBackend         string `json:"qobuz_backend,omitempty"`
	QobuzCountry         string `json:"qobuz_country,omitempty"`
	AppleMusicFallback   bool   `json:"apple_music_fallback,omitempty"`
	PreferAppleMusic     bool   `json:"prefer_apple_music,omitempty"`
}
...
func mapPlaybackQualityToTidal(quality string) string {
	switch strings.ToUpper(strings.TrimSpace(quality)) {
	case "HI_RES", "HI_RES_MAX", "HI_RES_LOSSLESS":
...
func ResolvePlaybackStreamJSON(requestJSON string) (string, error) {
	var req DownloadRequest
	if err := json.Unmarshal([]byte(requestJSON), &req); err != nil {
		return errorResponse("Invalid request: " + err.Error())
	}
	...
	case "qobuz":
		downloader := NewQobuzDownloader()
		track, err := resolveQobuzTrackForRequest(req, downloader, "QobuzStream")
		if err != nil {
			return errorResponse(err.Error())
		}
		...
		case "HI_RES":
			qobuzQuality = "7"
		case "HI_RES_LOSSLESS", "HI_RES_MAX", "", "DEFAULT":
			qobuzQuality = "27"
		}

		streamInfo, err := downloader.GetDownloadURLWithOptions(track.ID, qobuzQuality, req.QobuzBackend, req.QobuzCountry)
		if err != nil {
			return errorResponse("failed to resolve qobuz playback stream: " + err.Error())
		}

		bitDepth := track.MaximumBitDepth
		if streamInfo.BitDepth > 0 {
		...
		resp := map[string]any{
			"success":     true,
			"service":     "qobuz",
			"stream_url":  streamInfo.DownloadURL,
			...
			"track_id":    track.ID,
			"title":       track.Title,
			"artist":      qobuzTrackArtistName(track),
			"album":       track.Album.Title,
			"qobuz_id":    track.ID,
			"cover_url":   qobuzTrackAlbumImage(track),
		}
	...
	case "tidal":
		downloader := NewTidalDownloader()
		track, err := resolveTidalTrackForRequest(req, downloader, "TidalStream")
		if err != nil {
			return errorResponse(err.Error())
		}

		tidalQuality := mapPlaybackQualityToTidal(req.Quality)
		streamInfo, err := downloader.GetDownloadURL(track.ID, tidalQuality)
		if err != nil {
			return errorResponse("failed to resolve tidal playback stream: " + err.Error())
		}

		streamURL, mimeType, codecs, err := resolveTidalPlaybackStream(streamInfo, tidalQuality)
		if err != nil {
			return errorResponse("failed to prepare tidal playback stream: " + err.Error())
		}
		...
		resp := map[string]any{
			...
			"mime_type":   mimeType,
			"codecs":      codecs,
			"bit_depth":   streamInfo.BitDepth,
			"sample_rate": streamInfo.SampleRate,
			"track_id":    track.ID,
			"title":       strings.TrimSpace(track.Title),
			"artist":      tidalTrackArtistsDisplay(track),
			"album":       strings.TrimSpace(track.Album.Title),
			"tidal_id":    track.ID,
			"cover_url":   tidalImageURL(track.Album.Cover, "1280x1280"),
		}

		jsonBytes, _ := json.Marshal(resp)
		return string(jsonBytes), nil
	default:
		return errorResponse("Unsupported playback service: " + service)
	}
}
