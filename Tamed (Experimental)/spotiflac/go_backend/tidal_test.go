...
func TestResolveTidalPlaybackStream_DirectURL(t *testing.T) {
	...
	url, mimeType, codecs, err := resolveTidalPlaybackStream(info, "HI_RES_LOSSLESS")
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if url != info.URL {
	...
	if mimeType != "audio/flac" || codecs != "flac" {
		t.Fatalf("unexpected direct stream format: mime=%q codecs=%q", mimeType, codecs)
	}
}

func TestResolveTidalPlaybackStream_BTSManifest(t *testing.T) {
	manifest := `{"mimeType":"audio/flac","codecs":"flac","encryptionType":"NONE","urls":["https://cdn.example.com/track.flac"]}`
	info := TidalDownloadInfo{
		URL: "MANIFEST:" + base64.StdEncoding.EncodeToString([]byte(manifest)),
	}

	url, mimeType, codecs, err := resolveTidalPlaybackStream(info, "LOSSLESS")
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if url != "https://cdn.example.com/track.flac" {
		t.Fatalf("unexpected manifest url: %q", url)
	}
	if mimeType != "audio/flac" || codecs != "flac" {
		t.Fatalf("unexpected BTS stream format: mime=%q codecs=%q", mimeType, codecs)
	}
}
